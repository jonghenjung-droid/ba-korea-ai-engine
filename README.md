...
캠페인 실행이 끝나면(7개 엔진 완료 후) 5개 버튼이 나타납니다. 각 버튼은 지금까지 생성된 캠페인 데이터(Brand DNA·Personas·Strategy·ROAS·CLV 등) 전체를 컨텍스트로 넘겨, 해당 분야 "글로벌 전문가" 페르소나로 Claude를 호출합니다.

| 버튼 | 페르소나 | 산출물 |
|---|---|---|
| 15초 영상광고 스토리보드·콘티 | 칸 라이언즈 수상 CF 감독 | 씬별 타임코드·화면 설명·대사·사운드 디렉션 |
| 브랜드 네이밍 | Interbrand·Landor 수준 네이밍 전문가 | 네이밍 후보 5개 + 근거·국제화 고려사항 |
| 카피라이팅 | 데이비드 오길비 수준 카피라이터 | 매체별(배너·소셜·옥외·인쇄) 헤드라인·바디카피·CTA |
| 콘텐츠 마케팅 제안서 | 글로벌 콘텐츠 전략가 | 3개월 콘텐츠 필러·채널별 캘린더·KPI |
| 브랜드 마케팅 제안서 | Bain·McKinsey 브랜드 프랙티스 파트너 | Executive Summary·포지셔닝·3개년 로드맵·예산 근거 |

각 결과는 화면에서 바로 복사할 수 있고, 별도 파일로 저장되지는 않습니다(새로고침하면 사라짐 — 필요하면 복사해두세요). 마크다운 헤더(#/##)와 목록(-)을 사용해 응답하도록 지시했고, 화면에서 가벼운 자체 렌더러로 표시합니다.

**한계**: 이 5개는 실제 캠페인 데이터(브랜드 설명·타깃·MMM 배분 등)를 참고는 하지만, 최종적으로는 Claude가 "전문가라면 이렇게 만들 것이다"라는 생성 결과이지, 실제 수상 경력 전문가의 작업물이 아닙니다. 브랜드 네이밍은 특히 상표 등록 가능 여부를 반드시 별도로 법률 검토하시기 바랍니다.
