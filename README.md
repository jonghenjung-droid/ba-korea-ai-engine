...
캠페인 실행이 끝나면(7개 엔진 완료 후) 8개 버튼이 나타납니다. 상단 4개는 **BA KOREA가 실제로 크몽에서 판매 중인 상품과 1:1로 연결되는 무료 예비진단**이고, 하단 4개는 창작 실행물입니다.
...
| 제안서·RFP 심사 취약점 진단 | 정부·대기업 제안서 평가위원 출신 | 심사자 관점 취약점 4가지 | [떨어진 적 없는 제안서 선정 노하우](https://kmong.com/gig/807073) (199,000원~) |
| 사업계획서 관점 점검 포인트 | 정부지원사업 평가위원 출신 | 사업성·시장성·BM 관점 보완점 3가지 | [스타트업 멘토 출신 사업계획서 컨설팅](https://kmong.com/gig/806456) (19,900원~) |
| 행사·활성화 이벤트 리스크 점검 | 글로벌 이벤트 기획 실무자 | 컨셉·동선·예산·KPI 리스크 4가지 | [본부장 출신 국내외 이벤트 기획](https://kmong.com/gig/806963) (199,000원~) |
| 15초 영상광고 스토리보드·콘티 | 칸 라이언즈 수상 CF 감독 | 씬별 타임코드·화면 설명·대사·사운드 디렉션 | (연결 상품 없음 — 실행 자산) |
| 카피라이팅 | 데이비드 오길비 수준 카피라이터 | 매체별(배너·소셜·옥외·인쇄) 헤드라인·바디카피·CTA | (연결 상품 없음 — 실행 자산) |
...
**퍼널 설계 원칙**: 상단 3개는 실제 크몽 STANDARD 패키지(가장 저렴한 "간편 진단" 티어)와 같은 깊이로만 진단하도록 프롬프트에 명시했습니다 — 확정적 처방이 아니라 "힌트 수준"으로만 제한하고, 결과 하단에 실제 상담 링크(CTA 배너)를 자동으로 붙였습니다. 즉 AI가 무료로 맛보기를 보여주고, 더 정밀한 진단·실행은 유료 컨설팅으로 자연스럽게 이어지는 구조입니다.
