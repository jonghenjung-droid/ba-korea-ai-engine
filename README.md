...
| Media Engine | 브랜드 설명 기반 고정 휴리스틱(코사인 유사도) | 페르소나별 `lifestyle.primary_platforms.weight`를 `audience_share`로 가중 평균, 마케팅 목표에 해당하는 `journey_touchpoints` 단계(인지도→awareness, 전환/재구매→decision)의 접점 채널은 추가 가중 — 단순 발견 채널과 실제 결정 채널을 구분 |
...
- **채널명 명확화**: "메타 (Meta)"라는 채널명이 인스타그램을 포함한다는 걸 알기 어려워 "인스타그램 (Meta)"로 바꿨습니다.
- **업종별 SNS 이용 통계 웹 검색 기반 반영**: 처음엔 "여행·라이프스타일은 인스타그램이 틱톡보다 높다"는 고정 규칙 하나만 넣었는데, 업종·상품군마다 실제 SNS 이용 패턴이 다르다는 점(패션·뷰티·여행=인스타그램 중심, 게임·엔터=유튜브·틱톡 중심, B2B=네이버 검색 중심 등)을 반영해 고정 규칙 대신 **Customer Engine에 web_search를 활성화**했습니다. 채널별 weight를 정하기 전에 이 브랜드의 업종·타깃에 맞는 최신 국내 SNS 이용률 통계(오픈서베이·나스미디어·DMC미디어 등)를 직접 검색해 확인한 뒤 근거로 삼도록 지시했습니다. 검색 결과가 없거나 애매한 경우에만 페르소나 서술 기반으로 추정합니다.
  **정확한 표현으로 짚자면**: 이건 매초 갱신되는 라이브 API 통계가 아니라, Claude가 매 실행마다 웹 검색으로 찾아낸 "가장 최근에 공개된 조사자료"입니다. 고정된 규칙보다는 실제에 가깝지만, 방송통신위원회 통계 API 같은 완전한 실시간 연동은 아닙니다. (화면의 "엔진 작동 방식 자세히 보기" 토글에도 동일한 안내를 넣었습니다.)

## 진단 정확도 향상 — 자동 수집 + 선택 입력

- **브랜드 URL 자동 수집**: 사이드바에 URL을 입력하면 `/api/fetch-url`이 그 페이지의 본문 텍스트를 자동으로 긁어와 Brand Engine 프롬프트에 추가합니다. Firecrawl 같은 유료 API 없이 fetch + HTML 태그 제거로 구현한 경량 버전입니다. 자바스크립트 렌더링 사이트(SPA)는 원문이 비어있을 수 있습니다.
- **"브랜드 설명" 칸에 URL만 입력한 경우 자동 감지**: 실제 사례로 확인된 오류 — 설명 칸에 URL 하나만 달랑 입력하면 Brand Engine이 브랜드명 글자만 보고 엉뚱하게 추측하는 문제(예: "글램탐다오" 글램핑 브랜드를 뷰티 브랜드로 오인)가 있었습니다. 이제 설명 칸이 URL 하나로만 이루어져 있으면 자동으로 그 URL을 읽어오는 대상에 포함시킵니다.
- **소셜 채널 URL + 검색 스니펫 보조 수집**: 인스타그램·네이버플레이스 URL을 입력받되, 이런 페이지는 자바스크립트 렌더링이라 직접 읽기가 실패할 확률이 높습니다. 이 경우 `SERPAPI_KEY`가 설정되어 있으면 브랜드명으로 일반 검색을 돌려, 검색엔진이 이미 색인해둔 텍스트(리뷰·소개 일부)를 보조 근거로 사용합니다 (`/api/brand-search`).
- **선택 입력 3종**: 경쟁 브랜드, 지금 겪고 있는 문제, 기존 슬로건/소개 문구. 전부 선택사항이며, 채우면 Brand Engine과 모든 무료 진단·실행 산출물의 근거로 함께 사용됩니다.
- **Claude 자체 웹 검색 (신규, 가장 확실한 해결책)**: 실사용 중 "글램탐다오"(가평 소재 글램핑) 브랜드가 뷰티 브랜드로 오인되는 문제가 발견됐습니다. 원인은 `glamtamdao.co.kr`이 자바스크립트로 렌더링되는 사이트라 `/api/fetch-url`이 빈 텍스트만 받아왔기 때문입니다. 이 문제를 근본적으로 해결하기 위해, Brand Engine과 두 무료 진단(브랜드 방향·마케팅)에 Anthropic의 서버사이드 `web_search_20250305` 툴을 활성화했습니다. 입력 정보가 부실하면 Claude가 직접 브랜드명을 검색해 실제 정보를 확인한 뒤 분석합니다 — SerpAPI 키나 별도 크롤러 없이도, 어떤 사이트 구조든 우회할 수 있는 가장 신뢰도 높은 경로입니다. (참고: 이 기능은 배포 환경에서 실제 라이브 호출로 검증되지 않았습니다 — Anthropic 공식 문서 기준으로 구현했으니 배포 후 확인이 필요합니다.)
- **진단 출력물의 실시간 시장 데이터 연동**: 추적 키워드를 등록해두셨다면, "브랜드 방향 무료진단"·"마케팅 무료 진단" 생성 시에도 Strategy Engine과 동일하게 RAG로 수집된 실시간 데이터를 함께 참고합니다.

---

## 현재 버전의 알려진 한계 (다음 고도화 대상)

- **Brand Engine**: URL 자동 수집(경량 fetch 방식)이 추가됐지만, 정식 Firecrawl 같은 크롤링 서비스는 아니라 자바스크립트 렌더링 사이트에서는 텍스트를 못 가져올 수 있습니다.
